...
onsetIndex  = onset_SOP( 'data/bass/2_bassX_Billie Jean_Michael Jackson_BassGr1.wav')
df = df_array[1]
...
# Do the Students
for i in range(8): # Number of  student files  
    ...
    sprecision, srecal, sf_measure_value=evaluate_accuracy(onset_list, sonsetIndex, 0.02)
    print("\n")
...
onset_X, onset_mean, onset_std = write_sop_stem_stats(onsetIndex," onsets", p,r,f,"billie_sop_gt")

the_student_grades = returnGradesYellow()
print("the student_grades SOP")
print(len(the_student_grades))

sonset_X, sonset_mean, sonset_std = write_sop_stats(sonsetIndexArray," onsets", the_student_grades,sp,sr,sf,"billie")
