...
index= 1
while index <= 4:
  theFilename=student_path+'2_brown'+'/'+'brown'+str(index)+'b.wav'
...
onsetIndex  = onset_SOP(stem_filenames[3])
df = df_array[3]
...
# Do the Students
for i in range(4): # Number of yellow student files  
    sonsetIndex = onset_SOP(student_audio_name[i])
    print( "Just Looking: This is student number ", i+1, "Onset Len. length is ", len(sonsetIndex))
